...
const char head[] PROGMEM = {"GET /tracechain/api/newmsg?str="};
  ...
  for (unsigned long start = millis(); millis() - start < 30000;)  //一秒钟内不停扫描GPS信息
  {
    while (GPSSerial.available()) //串口获取到数据开始解析
    {
      ...
      case '$':         //若是$，则说明是一帧数据的开始
        ...
        if(gps.parseGprmcLat(nmeaSentence) > "")   //当不是空时候打印输出
          {
            loc += gps.parseGprmcLat(nmeaSentence);
            loc += ";";
          }
        if(gps.parseGprmcLon(nmeaSentence) > "")    //当不是空时候打印输出
        {
           loc += gps.parseGprmcLon(nmeaSentence);
        }
      }
    }
...
void loop() {
  ...
  Serial.println(http_cmd);
  i = 0;
  while(0 != gprs.connectTCP("139.220.193.149",80)){
